...
execute_with_logging() {
    ...
    log_message "INFO" "Rozpoczynam: $description"
    if eval "$command"; then
        log_message "SUCCESS" "Zakończono: $description"
        return 0
    else
        log_message "ERROR" "Błąd podczas: $description"
        return 1
    fi
}
...
check_rails_logs() {
    ...
    if [[ -f "$log_file" ]]; then
        print_info "Ostatnie błędy z logów Rails:"
        tail -n 50 "$log_file" | grep -A 5 "ERROR\|FATAL\|Error\|500" || {
            print_info "Nie znaleziono błędów w ostatnich logach"
        }
    else
            ...
            while true; do
                ...
                case $subchoice in
                    ...
                    3) # Rebuild/Debug
                        ...
                        print_info "Debugowanie Discourse (tryb developerski z logami)..."
                        ...
                        # Sprawdź i wyczyść procesy Rails
                        check_and_clean_rails_processes

                        # Sprawdź stan bazy danych
                        print_info "Sprawdzanie stanu bazy danych..."
                        execute_with_logging "cd '$DISCOURSE_DIR' && RAILS_ENV=development bundle exec rake db:migrate:status" "Sprawdzanie statusu migracji"

                        # Próba naprawy potencjalnych problemów
                        print_info "Próba naprawy potencjalnych problemów..."
                        execute_with_logging "cd '$DISCOURSE_DIR' && RAILS_ENV=development bundle exec rake assets:clean assets:precompile" "Przebudowa assetów"
                        execute_with_logging "cd '$DISCOURSE_DIR' && RAILS_ENV=development bundle exec rake db:migrate" "Aktualizacja bazy danych"

                        log_message "INFO" "Uruchamianie serwera w trybie debug"
                        if ! DISCOURSE_HOSTNAME=localhost \
                           UNICORN_LISTENER=localhost:3000 \
                           ALLOW_EMBER_CLI_PROXY_BYPASS=1 \
                           RAILS_ENV=development \
                           DISCOURSE_DEV_EAGER_LOAD=1 \
                           bundle exec rails server; then
                            log_message "ERROR" "Wystąpił błąd podczas uruchamiania serwera"
                            print_error "Wystąpił błąd podczas uruchamiania serwera"
                            check_rails_logs
                            read -rp "$(echo -e ${CYAN}Naciśnij Enter, aby kontynuować...${RESET})"
                            continue
                        fi

                        log_message "SUCCESS" "Debugowanie Discourse zostało uruchomione"
                        print_success "Debugowanie Discourse zostało uruchomione."
                        read -rp "$(echo -e ${CYAN}Naciśnij Enter, aby kontynuować...${RESET})"
                        ;;
                    4) # Zatrzymaj
